...
class Helper:
    def get_node_weights(node: Node) -> list[float]:
        return list(node.weights.values())
...
def test_output_layer_init_creates_output_layer_with_correct_number_of_nodes():
    prev_layer = InputLayer(2)
    layer = OutputLayer([[0, 0]] * 3, prev_layer)
    assert len(layer.nodes) == 3
...
def test_network_createEmpty_creates_valid_network_with_alternative_activation_function():
    def activation_function():
        pass

    network: Network = Network.createEmpty(activation_function)
    assert network.activation_function_f == activation_function
